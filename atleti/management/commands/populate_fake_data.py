...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # 1. PULIZIA PREVENTIVA: Rimuoviamo le attività precedenti di questi utenti per avere dati puliti
        self.stdout.write("Pulizia attività precedenti...")
        for p_data in profili_fake:
            try:
                u = User.objects.get(username=p_data['username'])
                Attivita.objects.filter(atleta__user=u).delete()
        ...
        for p_data in profili_fake:
            ...
            for i in range(40):
                ...
                # --- SABOTAGGIO PER TESTARE GLI ALLARMI ---
                # Mario Rossi ha un crollo di forma nelle ultime 5 sessioni (i < 5 sono le più recenti)
                if user.username == 'mario_amateur' and i < 5:
                    fc_media += 18 # +18 bpm a parità di passo = Crollo Efficienza VO2max
